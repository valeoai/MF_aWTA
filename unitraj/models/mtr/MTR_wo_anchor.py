...
import unitraj.models.mtr.loss_utils_awta as loss_utils
...
class MotionTransformer(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.pred_dicts = []
        self.model_cfg.MOTION_DECODER['use_wta'] = config['use_wta']
        if 'use_sigmoid' in config:
            self.model_cfg.MOTION_DECODER['use_sigmoid'] = True
        if 'random_init' in config:
            self.model_cfg.MOTION_DECODER['random_init'] = True
        if 'temperature' in config:
            self.model_cfg.MOTION_DECODER['temperature'] = config['temperature']
        if 'consider_velo' in config:
            self.model_cfg.MOTION_DECODER['consider_velo'] = config['consider_velo']
        else:
            self.model_cfg.MOTION_DECODER['consider_velo'] = False
        if 'dist_mode' in config:
            self.model_cfg.MOTION_DECODER['dist_mode'] = config['dist_mode']
        else:
            self.model_cfg.MOTION_DECODER['dist_mode'] = 'ade'
        if 'use_last' in config:
            self.model_cfg.MOTION_DECODER['use_last'] = config['use_last']
        else:
            self.model_cfg.MOTION_DECODER['use_last'] = False

        if 'binary' in config:
            self.model_cfg.MOTION_DECODER['binary'] = config['binary']
        else:
            self.model_cfg.MOTION_DECODER['binary'] = False

        if 'new_num_modes' in config:
            self.model_cfg.MOTION_DECODER['new_num_modes'] = config['new_num_modes']
    ...
    def forward(self, batch):
        #print("self.epoch_count", self.epoch_count)
...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        if 'new_num_modes' in config:
            self.num_motion_modes = config['new_num_modes']
        #xyh
        if 'temperature' in config:
            self.register_buffer('temperature', torch.zeros(1).float().cuda() + config["temperature"])
            self.register_buffer('init_temperature', torch.zeros(1).float().cuda() + config["temperature"]) 
        else:
            self.register_buffer('temperature', torch.zeros(1).float().cuda() + 1000.0)
            self.register_buffer('init_temperature',torch.zeros(1).float().cuda() + 1000.0) 
        print('init_temperature', self.init_temperature)
    ...
    def build_motion_query(self, d_model, use_place_holder=False):
        ...
        if use_place_holder:
            ...
            for cur_type in self.object_type:
                ...
                if self.num_motion_modes != 64:
                    # kmeans from 64 to 6 modes, intention points
                    #print("cur_intention_points.shape ", cur_intention_points.shape)
                    kmeans = KMeans(n_clusters=self.num_motion_modes, random_state=42, n_init="auto")
                    labels = kmeans.fit_predict(cur_intention_points)
                    #print(labels)
                    new_cur_intention_points = np.zeros_like(cur_intention_points[:self.num_motion_modes,:])
                    for label in range(self.num_motion_modes):
                        new_cur_intention_points[label,:] = np.mean(cur_intention_points[labels==label, :], axis=0)
                    print("new_cur_intention_points.shape ", new_cur_intention_points.shape)
                else:
                    new_cur_intention_points = cur_intention_points
                cur_intention_points = torch.from_numpy(new_cur_intention_points).float().view(-1, 2)
            ...
            if self.random_init:
                intention_query = nn.Embedding(self.num_motion_modes, d_model)
    ...
    def get_motion_query(self, center_objects_type):
        ...
        if self.use_place_holder:
            ...
            if self.random_init:
                #print('random init')
                intention_query = self.intention_query.weight.view(-1, 1, self.d_model).repeat(1,num_center_objects,1)
                #print(intention_query.shape)
            else:
                intention_query = position_encoding_utils.gen_sineembed_for_position(intention_points,
                                                                                 hidden_dim=self.d_model)
                #print(intention_query.shape)
            intention_query_out = self.intention_query_mlps(intention_query.view(-1, self.d_model)).view(-1,
                                                                                                     num_center_objects,
                                                                                                     self.d_model)  # (num_query, num_center_objects, C)
        return intention_query_out, intention_points
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        if not self.use_place_holder:
            ...
            if True:
                if self.model_cfg["use_last"] and self.model_cfg["use_last"]:
                    pred_scores, pred_trajs = pred_list[-1]
                    #scores = pred_scores.detach().clone()
                    trajs = pred_trajs.detach().clone()
                    my_gt_trajs = center_gt_trajs[:, :, 0:2].clone() 
                    #print("calculate nearest_mode_idxs based on predictions.")
                    my_distance = (trajs[:, :, :, 0:2].detach() - my_gt_trajs[:, None, :, :]).norm(dim=-1) #norm over (x,y) [batch, num_modes, timestamps]
                    if self.model_cfg['dist_mode'] =='fde':
                        last_valid_idx = center_gt_trajs_mask.sum(-1).clone() - 1
                        last_valid_idx = last_valid_idx.view(-1,1,1).repeat(1,my_distance.shape[1],1).long()
                        my_distance = torch.gather(my_distance, -1, last_valid_idx)[:,:,0] #take the last avaiable timestep
                        #print("fde mode: ", distance.shape)
                    else:
                        my_distance = (my_distance * center_gt_trajs_mask[:, None, :]).sum(dim=-1) #sum over timestamps [batch, num_modes]
                    #print('use last 1:', my_distance.sum())
                else:
                    my_distance = None
                
            if self.model_cfg["use_wta"]:
                # placeholder, not used
                #print(self.temperature)
                wta_weights = torch.softmax(-1.0*dist/self.temperature, dim=-1).detach() #(batch, num_query)
            else:
                #print("I am here!!!!!!!!!!!!!!!!!!!")
                wta_weights = None
        else:
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            pred_trajs_gmm, pred_vel = pred_trajs[:, :, :, 0:5], pred_trajs[:, :, :, 5:7] # pred_vel shape [batch, #modes, num_timestamps, 2] 
            
            if 'consider_velo' in self.model_cfg and self.model_cfg['consider_velo']:
                velo_dist = (pred_vel.detach().clone() - center_gt_trajs[:, None, :, 2:4]).norm(dim=-1) # [batch, #modes, num_timestamps]
                if self.model_cfg['dist_mode'] == 'ade':
                    velo_dist = (velo_dist * center_gt_trajs_mask[:, None, :]).sum(dim=-1)
                    velo_distance = velo_dist.detach()/center_gt_trajs_mask.sum(-1)[:,None] # (batch, num_modes)
                else: #fde
                    last_valid_idx = center_gt_trajs_mask.sum(-1).clone() - 1
                    last_valid_idx = last_valid_idx.view(-1,1,1).repeat(1,velo_dist.shape[1],1).long()
                    velo_distance = torch.gather(velo_dist.detach(), -1, last_valid_idx)[:,:,0] #take the last avaiable timestep
            else:
                velo_distance = None
            loss_reg_gmm, loss_reg_gmm_classic, center_gt_positive_idx, wta_weights = loss_utils.nll_loss_gmm_direct(
                pred_scores=pred_scores, pred_trajs=pred_trajs_gmm,
                gt_trajs=center_gt_trajs[:, :, 0:2], gt_valid_mask=center_gt_trajs_mask,
                pre_nearest_mode_idxs= None, #center_gt_positive_idx,
                timestamp_loss_weight=None, use_square_gmm=False, temperature=self.temperature, wta_weights=wta_weights, dist_mode=self.model_cfg['dist_mode'], my_distance=my_distance, velo_distance=velo_distance)

            # pred_vel : (batch_size, num_modes, num_timestamps, 2)
            ...
            if True:
                #print("enter None")
                pred_vel_bis = pred_vel[torch.arange(num_center_objects), center_gt_positive_idx]
                loss_reg_vel = F.l1_loss(pred_vel_bis, center_gt_trajs[:, :, 2:4], reduction='none')
                loss_reg_vel = (loss_reg_vel * center_gt_trajs_mask[:, :, None]).sum(dim=-1).sum(dim=-1)
                loss_reg_vel_classic = loss_reg_vel.detach().clone()
                #loss_cls = F.cross_entropy(input=pred_scores, target=center_gt_positive_idx, reduction='none')
            if wta_weights is not None:
                del loss_reg_vel
                #print("in AWTA for vel")
                assert wta_weights.shape[0] == pred_vel.shape[0] and wta_weights.shape[1] == pred_vel.shape[1]
                loss_reg_vel = 0.0
                #loss_cls = 0.0
                for mode_index in range(wta_weights.shape[1]):
                    wta_weight = wta_weights[:, mode_index]
                    pred_vel_tmp = pred_vel[:, mode_index] # (batch_size, num_timestamps, 3)
                    loss_reg_vel_tmp = F.l1_loss(pred_vel_tmp, center_gt_trajs[:, :, 2:4], reduction='none')
                    loss_reg_vel += (loss_reg_vel_tmp * center_gt_trajs_mask[:, :, None]).sum(dim=-1).sum(dim=-1)*wta_weight
            
            #TODO to implement
            #print(center_gt_positive_idx)
            #print(center_gt_positive_idx.requires_grad)
            if not self.use_sigmoid:
                loss_cls = F.cross_entropy(input=pred_scores, target=center_gt_positive_idx, reduction='none')
            # soft classification loss #
            else:
                #print('use sigmoid')
                #if wta_weights is None:
                #if self.model_cfg['dist_mode'] == 'ade':
                if my_distance is not None:
                    my_distance_bis = my_distance.detach().clone()
                    if not self.model_cfg['dist_mode'] == 'ade':
                        avg_distance = my_distance
                    else:
                        avg_distance = my_distance_bis/center_gt_trajs_mask.sum(-1)[:,None] # (batch, num_modes)
                else:
                    trajs = pred_trajs.detach().clone()
                    my_gt_trajs = center_gt_trajs[:, :, 0:2].clone()
                    my_distance_bis = (trajs[:, :, :, 0:2].detach() - my_gt_trajs[:, None, :, :]).norm(dim=-1) #norm over (x,y) [batch, num_modes, timestamps]
                    #my_distance_bis = (my_distance_bis * center_gt_trajs_mask[:, None, :]).sum(dim=-1)

                    if not self.model_cfg['dist_mode'] == 'ade':
                        last_valid_idx = center_gt_trajs_mask.sum(-1).clone() - 1
                        last_valid_idx = last_valid_idx.view(-1,1,1).repeat(1,my_distance_bis.shape[1],1).long()
                        avg_distance = torch.gather(my_distance_bis, -1, last_valid_idx)[:,:,0] #take the last avaiable timestep
                ...
                wta_weights = torch.softmax(-1.0*avg_distance/temperature, dim=-1).detach() #(batch, num_query)
                #print("wta_weights.shape ", wta_weights.shape)
                p = torch.nan_to_num(F.logsigmoid(pred_scores))
                minus_p = torch.nan_to_num(torch.log(1.0-F.sigmoid(pred_scores)))
                w_labels = wta_weights.detach()
                if self.model_cfg["binary"]:
                    w_labels = torch.zeros_like(wta_weights.detach())
                    #print("use binary", w_labels.shape)
                    for batch_idx in range(wta_weights.shape[0]):
                    
                        w_labels[batch_idx, torch.argmax(wta_weights[batch_idx])] = 1.0



                #print("w_labels[0] ", w_labels[0])
                #print("w_labels.sum(-1) ", w_labels.sum(-1))
                loss_cls = (-(w_labels*p) - (1.0 - w_labels) * minus_p).sum(-1) 
            ...
            tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_gmm_classic'] = loss_reg_gmm_classic.mean().item() * weight_reg
            tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_vel_classic'] = loss_reg_vel_classic.mean().item() * weight_vel
    ...
    def get_dense_future_prediction_loss(self, tb_pre_tag='', tb_dict=None, disp_dict=None):
        ...
        loss_reg_gmm, _,  _, _ = loss_utils.nll_loss_gmm_direct(
    ...
    def generate_final_prediction(self, pred_list, batch_dict):
        ...
        if False and 6 != num_query: #self.num_motion_modes != num_query:
            assert num_query >= 6 # self.num_motion_modes
            pred_trajs_final, pred_scores_final, selected_idxs = motion_utils.batch_nms(
                ...
                num_ret_modes=6 #self.num_motion_modes
            )
        else:
            pred_trajs_final = pred_trajs
            pred_scores_final = pred_scores
        #print("pred_trajs_final", pred_trajs_final.shape)
        #print("pred_scores_final", pred_scores_final.shape)
