...
class BaseModel(pl.LightningModule):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.my_predict_step = 0

        self.pred_dicts = []
        if 'epoch_count' in self.config:
            self.epoch_count = self.config['epoch_count']
        else:
            self.epoch_count = 0
    ...
    def training_step(self, batch, batch_idx):
        prediction, loss, loss_dict = self.forward(batch)
        self.compute_official_evaluation(batch, prediction)
        self.log_info(batch, batch_idx, prediction, loss_dict, status='train')
        return loss

    def validation_step(self, batch, batch_idx):
        prediction, loss, loss_dict = self.forward(batch)
        self.compute_official_evaluation(batch, prediction)
        self.log_info(batch, batch_idx, prediction, loss_dict, status='val')
        return loss
    
    def predict_step(self, batch, batch_idx):
        prediction, loss, loss_dict = self.forward(batch)
        self.compute_official_evaluation_bis(batch, prediction)
        '''
        print("self.my_predict_step ", self.my_predict_step)
        os.makedirs(f"./submission/{self.ckpt_name}", exist_ok=True)
        for real_batch_idx in range(len(prediction)) :
            fig = visualization.visualize_prediction(batch, prediction, real_batch_idx)
            save_path = f"./submission/{self.ckpt_name}/{str(self.my_predict_step).zfill(4)}{str(real_batch_idx).zfill(3)}.png"
            fig.savefig(save_path, format='png', dpi=300)
        self.my_predict_step += 1
        '''


    def on_predict_epoch_end(self):
        all_preds = self.pred_dicts
        ...
        for pred_dict in all_preds:
            gt_traj.append(pred_dict['gt_traj'])
            gt_traj_mask.append(pred_dict['gt_traj_mask'])
            center_gt_final_valid_idx.append(pred_dict['center_gt_final_valid_idx'])
            predicted_traj.append(pred_dict['pred_trajs'])
            predicted_prob.append(pred_dict['pred_scores'])

        gt_traj = np.stack(gt_traj, axis=0)
        gt_traj_mask = np.stack(gt_traj_mask,axis=0)
        center_gt_final_valid_idx = np.stack(center_gt_final_valid_idx,axis=0)
        predicted_traj = np.stack(predicted_traj, axis=0)
        predicted_prob = np.stack(predicted_prob, axis=0)
        predictions = self.compute_metrics(torch.from_numpy(gt_traj), torch.from_numpy(gt_traj_mask), torch.from_numpy(center_gt_final_valid_idx), torch.from_numpy(predicted_traj), torch.from_numpy(predicted_prob))
    ...
    def compute_official_evaluation_bis(self, batch_dict, prediction):
        ...
        #gt info for eval#
        gt_traj = input_dict['center_gt_trajs'].unsqueeze(1)  # .transpose(0, 1).unsqueeze(0)
        gt_traj_mask = input_dict['center_gt_trajs_mask'].unsqueeze(1)
        center_gt_final_valid_idx = input_dict['center_gt_final_valid_idx']
        pred_dict_list = []
        for bs_idx in range(batch_dict['batch_size']):
            single_pred_dict = {
                            'scenario_id': input_dict['scenario_id'][bs_idx],
                            'pred_trajs_world': pred_trajs_world[bs_idx, :, :, 0:2].cpu().numpy(),
                            'gt_trajs_world': input_dict['center_gt_trajs_src'][bs_idx].cpu().numpy(),
                            'pred_trajs': pred_trajs[bs_idx, :, :, 0:2].cpu().numpy(), # [64 modes, 60 timesteps, 2=(x,y)]
                            'pred_scores': pred_scores[bs_idx, :].cpu().numpy(), # [64 modes]
                            'object_id': input_dict['center_objects_id'][bs_idx],
                            'object_type': input_dict['center_objects_type'][bs_idx],
                            'gt_traj': gt_traj[bs_idx, :, :, 0:2].cpu().numpy(), # [1, 60 timesteps, 2=(x,y)], for eval
                            'gt_traj_mask': gt_traj_mask[bs_idx].cpu().numpy(), # [1, 60 timesteps], for eval, indicating valid future
                            'center_gt_final_valid_idx': center_gt_final_valid_idx[bs_idx].cpu().numpy(), #[1], for eval, indicating valid future
                            'track_index_to_predict': input_dict['track_index_to_predict'][bs_idx].cpu().numpy(),
                            # for plotting
                            #'map_polylines': input_dict['map_polylines'][bs_idx].cpu().numpy(),
                            #'map_polylines_mask': input_dict['map_polylines_mask'][bs_idx].cpu().numpy(),
                                }
    ...
    def on_train_epoch_end(self):
        ...
        try:
            print("self.motion_decoder.temperature ", self.motion_decoder.temperature)
            print("self.motion_decoder.init_temperature ", self.motion_decoder.init_temperature)
            if 'temperature_decay' in self.config:
                if 'linear' in self.config and self.config['linear']:
                    print("linear")
                    self.motion_decoder.temperature = self.motion_decoder.init_temperature - self.motion_decoder.init_temperature * self.epoch_count/100.0
                    if self.motion_decoder.temperature < 1e-8 :
                        self.motion_decoder.temperature = 1e-8
                else:
                    self.motion_decoder.temperature *= self.config['temperature_decay']
                    if self.motion_decoder.temperature < 1e-8 :
                        self.motion_decoder.temperature = torch.zeros(1).float().cuda() + 1e-8
            else:
                self.motion_decoder.temperature *= 0.834
                if self.motion_decoder.temperature < 1e-8 :
                    self.motion_decoder.temperature = torch.zeros(1).float().cuda() + 1e-8
        except:
            pass

    def compute_metrics(self, gt_traj, gt_traj_mask, center_gt_final_valid_idx, predicted_traj, predicted_prob):
        ...
        # weightedADE, weightedFDE
        wADE = (predicted_prob * ade_losses).sum(-1)
        wFDE =  (predicted_prob * fde).sum(-1)
        ...
        loss_dict = {
                    f'minADE{modes}': np.mean(minade),
                    f'minFDE{modes}': np.mean(minfde),
    ...
    def log_info(self, batch, batch_idx, prediction, my_loss_dict=None, status='train'):
        ## logging
        ...
        ade_diff = torch.norm(predicted_traj[:, :, :, :2] - gt_traj[:, :, :, :2], 2, dim=-1)
        ade_losses = torch.sum(ade_diff * gt_traj_mask, dim=-1) / torch.sum(gt_traj_mask, dim=-1)
        ade_losses = ade_losses.cpu().detach().numpy() #bs, #modes
        minade = np.min(ade_losses, axis=1)
        # Calculate FDE losses
        bs, modes, future_len = ade_diff.shape
        center_gt_final_valid_idx = center_gt_final_valid_idx.view(-1, 1, 1).repeat(1, modes, 1).to(torch.int64)

        fde = torch.gather(ade_diff, -1, center_gt_final_valid_idx).cpu().detach().numpy().squeeze(-1) # bs, #modes
        minfde = np.min(fde, axis=-1)
        ...
        loss_dict = {
            f'minADE{modes}': minade,
            f'minFDE{modes}': minfde,
        ...
        if my_loss_dict is not None:
            loss_dict.update(my_loss_dict)
        ...
        for k, v in loss_dict.items():
            ...
            self.log(status + "--" + k, v, on_step=False, on_epoch=True, sync_dist=True, batch_size=size_dict[k])
        try:
            self.log(status + "/"+'temperature', self.motion_decoder.temperature, on_step=False, on_epoch=True)
        except:
            pass
        if status == 'val' and batch_idx <0 and not self.config.debug:
            fig = visualization.visualize_prediction(batch, prediction)
            io_buf = io.BytesIO()
            ...
            tensorboard.add_image(status + "/"+ f"pred_{batch_idx}", img, dataformats='HWC', global_step=self.global_step)
