...
class Wayformer(BaseModel):
    ...
    def __init__(self, config):
        ...
        if 'temperature' in config:
            self.motion_decoder = Placeholder(self.config['temperature'])
        else:
            self.motion_decoder = Placeholder(100)
        print("self.motion_decoder.temperature ", self.motion_decoder.temperature)
        if 'use_sigmoid' in self.config:
            self.use_sigmoid = self.config['use_sigmoid']
        else:
    ...
    def _forward(self, inputs):
        ...
        # Mode prediction
        if 'stop_grad' in self.config and self.config['stop_grad']:
            #print("stop grad")
            mode_probs = self.prob_predictor(out_seq[:, :self.c].detach()).reshape(B, self.c)
        else:
    ...
    def forward(self, batch):
        ...
        ground_truth = torch.cat([inputs['center_gt_trajs'][..., :2], inputs['center_gt_trajs_mask'].unsqueeze(-1)],
                                 dim=-1)
        # xyh
        loss, loss_dict = self.criterion(output, ground_truth, inputs['center_gt_final_valid_idx'], temperature=self.motion_decoder.temperature)
        output['dataset_name'] = inputs['dataset_name']
        # after loss
        if self.use_sigmoid:
            output['predicted_probability'] = torch.sigmoid(output['predicted_probability'])
        else:
...
class Placeholder(nn.Module):
    def __init__(self, temperature):
        ...
        self.register_buffer('temperature', torch.zeros(1).float().cuda() + temperature)
        self.register_buffer('init_temperature', torch.zeros(1).float().cuda() + temperature)
...
class Criterion(nn.Module):
    ...
    def forward(self, out, gt, center_gt_final_valid_idx, temperature=None):
        return self.nll_loss_gmm_direct(out['predicted_probability'], out['predicted_trajectory'], gt,
                                        center_gt_final_valid_idx, temperature=temperature)
    ...
    def nll_loss_gmm_direct(self, pred_scores, pred_trajs, gt_trajs, center_gt_final_valid_idx,
                            ...
                            rho_limit=0.5, temperature=None):
        """
        ...
        gt_valid_mask = gt_trajs[..., -1] # (batch_size, num_timestamps)

        if pre_nearest_mode_idxs is not None:
            nearest_mode_idxs = pre_nearest_mode_idxs
            wta_weights = None
            wta_weights_wo_temp = None
            assert False # Should not enter here.
        else:
            if 'use_wta' in self.config and self.config['use_wta']:
                distance = (pred_trajs[:, :, :, 0:2].detach() - gt_trajs[:, None, :, :2]).norm(dim=-1) #norm over (x,y) [batch, num_modes, timestamps]
            else:
                distance = (pred_trajs[:, :, :, 0:2] - gt_trajs[:, None, :, :2]).norm(dim=-1) #norm over (x,y) [batch, num_modes, timestamps]
            # xyh
            if dist_mode == 'fde':
                #print('fde')
                last_valid_idx = gt_valid_mask.sum(-1) - 1
                last_valid_idx = last_valid_idx.view(-1,1,1).repeat(1,distance.shape[1],1).long()
                center_gt_final_valid_idx_bis = center_gt_final_valid_idx.view(-1, 1, 1).repeat(1, distance.shape[1], 1).to(torch.int64)
                #print("distance.shape ", distance.shape) 
                #print("last_valid_idx.shape", last_valid_idx.shape)
                #print("center_gt_final_valid_idx.shape ", center_gt_final_valid_idx.shape)
                distance = torch.gather(distance, -1, center_gt_final_valid_idx_bis)[:,:,0] #take the last avaiable timestep, # batch, #modes
                #print(distance.shape)
            else:
                distance = (distance * gt_valid_mask[:, None, :]).sum(dim=-1) # [batch, num_modes]

            nearest_mode_idxs = distance.argmin(dim=-1)

            # xyh
            if True: #'use_wta' in self.config and self.config['use_wta']:
                ...
                wta_weights_wo_temp = torch.softmax(-1.0*avg_distance, dim=-1).detach() #(batch, num_modes)
            else:
                wta_weights = None
                wta_weights_wo_temp = None
        ...
        # wta version, xyh
        if wta_weights is not None and 'use_wta' in self.config and self.config['use_wta']:
            #print("use_wta")
            del reg_loss
        ...
        if not use_sigmoid:
            ...
            if 'use_temp' in self.config and self.config['use_temp']:
                #print('use_temp')
                w_labels = wta_weights.detach()
            else:
                w_labels = wta_weights_wo_temp.detach()
            # if self.config["binary"]:
            #     w_labels = torch.zeros_like(wta_weights_wo_temp.detach())
            #     #print("use binary", w_labels.shape)
            #     for batch_idx in range(wta_weights_wo_temp.shape[0]):
                
            #         w_labels[batch_idx, torch.argmax(wta_weights_wo_temp[batch_idx])] = 1.0
        ...
        loss_dict = {
                ...
                'loss_reg': reg_loss_classic.mean().item(),
                'loss_reg_wta': reg_loss.mean().item()
                }
        return (reg_loss + loss_cls).mean(), loss_dict
